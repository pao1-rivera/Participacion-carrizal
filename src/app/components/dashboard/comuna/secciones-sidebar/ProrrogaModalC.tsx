...
interface ProyectoComunaSimple {
...
interface ProrrogaModalCProps {
  isOpen: boolean;
  onClose: () => void;
  proyectoName: string;
  proyectos: ProyectoComunaSimple[];
  onSelectProyecto: (proyecto: ProyectoComunaSimple) => void;
  selectedProyecto: ProyectoComunaSimple | null;
...
export const ProrrogaModalC = ({ 
  ...
  const [voceros, setVoceros] = useState<{ id_voceroc: number; nombre_completo: string; cedula: string }[]>([]);
  ...
  const [existingFilePaths, setExistingFilePaths] = useState<{ acta_url: string | null; evidencia_url: string | null }>({
    acta_url: null,
    evidencia_url: null,
  });
  ...
  // Obtener id_comuna del usuario logueado
  useEffect(() => {
    const fetchComunaId = async () => {
      ...
      const { data, error } = await supabase
        ...
        .maybeSingle();
      if (data) setComunaId(data.id_comuna);
    };
    fetchComunaId();
  }, [user]);

  // Cargar voceros firmantes de la comuna
  useEffect(() => {
  ...
  useEffect(() => {
    if (isOpen && editMode && initialData) {
      const proyectoEdit = proyectos.find(p => p.id_proyecto_comuna === initialData.id_proyecto_comuna);
  ...
  const uploadFile = async (file: File, folder: string): Promise<string | null> => {
    ...
    const filePath = `comuna_${comunaId}/prorrogas/${folder}/${fileName}`;
    const { error } = await supabase.storage.from('rendiciones_comuna').upload(filePath, file);
    if (error) {
  ...
  const deleteOldFile = async (filePath: string | null) => {
    if (!filePath) return;
    const { error } = await supabase.storage.from('rendiciones_comuna').remove([filePath]);
    ...
    try {
      ...
      if (editMode && initialData?.id_solicitud) {
        ...
        if (onProrrogaAccepted && selectedProyecto) {
          onProrrogaAccepted({ id_proyecto: selectedProyecto.id_proyecto_comuna, nombre: selectedProyecto.nombre, codigo: selectedProyecto.codigo });
        }
      } else {
        const { error: insertError } = await supabase
          .from('solicitudes_prorroga_comuna')
          .insert({
            id_proyecto_comuna: selectedProyecto.id_proyecto_comuna,
            id_comuna: comunaId,
            motivo,
            presupuesto_estimado: presupuestoEstimado,
            tiempo_estimado: tiempoEstimado,
              ...
              {step === 1 && (
                <div className="space-y-4">
                  <div>
                    ...
                    <select 
  value={selectedProyecto?.id_proyecto_comuna || ''} 
  onChange={(e) => {
    const proyecto = proyectos.find(p => p.id_proyecto_comuna === parseInt(e.target.value));
...
>
  ...
  {proyectos
    .filter(p => !editMode ? (p.estado?.toLowerCase().trim() === 'en ejecución') : true)
    .map(p => (
      <option key={p.id_proyecto_comuna} value={p.id_proyecto_comuna}>
                  ...
                  <div>
                    ...
                    <div className="max-h-48 overflow-y-auto space-y-2 border border-gray-100 rounded-xl p-2 bg-gray-50/50">
                      {voceros.length === 0 ? (
                        ...
                        voceros.map(vocero => (
                          <label key={vocero.id_voceroc} className="flex items-center gap-2 p-2 rounded-lg bg-white border border-gray-100 cursor-pointer hover:bg-gray-50">
                            <input
                              type="checkbox"
                              checked={selectedVocerosIds.includes(vocero.id_voceroc)}
                              onChange={() => toggleVocero(vocero.id_voceroc)}
